...
export default function PredictionDetail() {
  ...
  return <div className="detail-page">
    ...
    <div className="detail-columns"><main>
      ...
      <section className="detail-section"><div className="section-heading"><span className="panel-index">03</span><div><h2>Evidence</h2><p>Sources attached to this forecast.</p></div></div>{prediction.evidence.length ? prediction.evidence.map(item => <a className="evidence-row" href={item.source_url} target="_blank" rel="noreferrer" key={item.id}><div><span className={`evidence-stance ${item.stance}`}>{item.stance}</span><h3>{item.title}</h3><p>{item.excerpt}</p><small>{item.source_name} · {item.published_at ? date(item.published_at) : "undated"}</small></div><ExternalLink size={15} /></a>) : <div className="evidence-empty"><Link2 size={18} /><div><h3>No external sources attached.</h3><p>No external sources are attached to this forecast.</p></div></div>}</section>
