...
export default function AuthDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={() => onOpenChange(false)}><section className="auth-modal" onMouseDown={event => event.stopPropagation()}><button className="modal-close" onClick={() => onOpenChange(false)} aria-label="Close"><X size={18} /></button><div className="eyebrow">Private forecast record</div><h2>{mode === "sign-in" ? "Sign in" : "Create your account"}</h2><p className="modal-intro">Save forecasts, review changes, and compare predictions with outcomes.</p><div className="auth-tabs"><button className={mode === "sign-in" ? "selected" : ""} onClick={() => setMode("sign-in")}>Sign in</button><button className={mode === "sign-up" ? "selected" : ""} onClick={() => setMode("sign-up")}>Create account</button></div><form onSubmit={submit} className="auth-form"><label><span><Mail size={14} /> Email</span><input type="email" value={email} onChange={event => setEmail(event.target.value)} required placeholder="you@example.com" /></label><label><span><LockKeyhole size={14} /> Password</span><input type="password" value={password} onChange={event => setPassword(event.target.value)} required minLength={6} placeholder="At least 6 characters" /></label>{message && <div className="form-message">{message}</div>}<button className="primary-button full" disabled={busy}>{busy ? "Working…" : mode === "sign-in" ? "Sign in" : "Create account"}<ArrowRight size={16} /></button></form><p className="auth-footnote">Your saved forecasts are private to your account.</p></section></div>;
}
